...
export class ChatComponent implements OnInit {
  ...
  openChat(user: any) {
    ...
    if (currentUserId) {
      // Form the chatId based on the current user and selected user
      const chatId = [currentUserId, user.id].sort().join('_');
      this.chatService.getMessages(chatId).subscribe((data: any) => {
        this.messages = data;
      });
    } else {
  ...
  sendMessage() {
    if (this.messageForm.valid && !this.sendingMessage) { // Check form validity and if a message is not already being sent
      ...
      if (currentUserId) {
        // Form the chatId based on the current user and selected user
        const chatId = [currentUserId, this.selectedUser.id].sort().join('_');
        const message = {
        ...
        this.chatService.sendMessage(chatId, message).then(() => {
          this.newMessage = ''; 
